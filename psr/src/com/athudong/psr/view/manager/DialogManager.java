...
public class DialogManager {
	...
	/**以对话框的方式弹出时间选择器*/
	public static DatePickWheel ShowDateSelectedDialog(Activity activity,final TextView tv,boolean justHourMinue,final TextView tv2,final int hour,final int minute){
		final Dialog dialog = new Dialog(activity, R.style.mDialog);
		Window window = dialog.getWindow();
		window.setBackgroundDrawableResource(android.R.color.transparent);
		...
		sure.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				tv.setText(dpw.getDate());
				tv2.setText(dpw.getIndexDate(hour,minute));
				dialog.dismiss();
			}
		});
		dialog.setCanceledOnTouchOutside(false);
